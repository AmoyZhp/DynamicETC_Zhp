from env.static_graph import StaticGraph, Node, Edge, AdjacencyNode
import logging
...
class TrafficGraph(StaticGraph):
    ...
    def _init_graph(self, node_dict_list, edge_dict_list):
        ...
        for node_dict in node_dict_list:
            # 这里的 node 是 Zone
            zone_id = len(self.nodes_list)
            zone_label = node_dict.get('label', "")
            zone = Zone(zone_id, zone_label)
            temp_node_key_id_map[node_dict['node_id']] = zone.node_id
            self.nodes_list.append(zone)
            self.nodes_adjacency_list.append(AdjacencyNode(zone))
    ...
    def data(self):
        """将该类转换成一个字典，包含 roads 和 zones 信息
        
        Returns:
            dict -- 包含 roads 和 zones 的信息
        """
        roads_list = []
        zones_list = []
        for zone in self.nodes_list:
            zones_list.append(zone.__dict__)
        
        for road in self.edges_list:
            roads_list.append(road.__dict__)
        
        return {'zones': zones_list, 'roads': roads_list, 'super':super().data()}
...
class Path():
    ...
    def data(self):
        ...
        for road in self.roads:
            roads_dict.append(road.__dict__)
        return {
            'roads': roads_dict,
            'origin': self.origin,
...
class Road(Edge):
    ...
    def init_length(self, length):
        ...
        self.free_flow_travel_time = self.length * 2
